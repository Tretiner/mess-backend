...
fun initDatabase(config: DatabaseConfig) {
    Database.connect(url = config.url, driver = "org.postgresql.Driver", user = config.user, password = config.password)
    transaction {
        // Создаем только AuthUsersTable
        SchemaUtils.create(AuthUsersTable)
        // RefreshTokensTable УДАЛЕНА.
    }
}